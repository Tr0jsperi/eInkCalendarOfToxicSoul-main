...
GxEPD2_7C < GxEPD2_565c, GxEPD2_565c::HEIGHT / 2 > display(GxEPD2_565c(/*CS=5*/ 15, /*DC=*/ 27, /*RST=*/ 26, /*BUSY=*/ 25)); // Waveshare 5.65" 7-color
#endif
...
#define TIME_TO_SLEEP 60 * 10 * 1  /* 改变这里的值来调整你需要的休眠时间，这里最终的值为秒。 例如 60*60*3 最终会休眠3个小时 */ 
...
void ShowPage(PageContent pageContent)
{
   ...
   * @tr0jsperi 全局刷新屏幕只能显示一次，如果下面代码执行，将会导致页面只显示天气图标
   */
  /*
  switch (pageContent)
  {
